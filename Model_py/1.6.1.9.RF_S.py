...
def RF_run(train, test, feature_lists, output_name1):
    ...
    grid_search = GridSearchCV(rf_model, param_grid=fr_param_grid, cv=5, return_train_score = True, scoring='roc_auc', n_jobs=40)
...
result.to_csv(path + 'Thesis/univ_markers_RF_9_S.csv')
result2.to_csv(path + 'Thesis/total_maseed_pathrkers_RF_9_S.csv')

for d_path in outer_path:
    for seed_num in range(11,51):
        seed_path = d_path + 'seed' + str(seed_num) + '/'
        commands = "mkdir " + seed_path + "RF & mv " + seed_path + "*.sav " + seed_path
+ "RF/"
        os.system(commands)
